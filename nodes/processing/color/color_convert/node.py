...
def _build_conversion_map():
    ...
    pairs = {}
    # Mapping from lowercase param value to cv2 name
    param_to_cv2 = {
        "bgr": "BGR",
        "rgb": "RGB",
        "hsv": "HSV",
    ...
    # Get all parameter names (lowercase)
    param_names = list(param_to_cv2.keys())
    
    for src_param in param_names:
        for tgt_param in param_names:
            if src_param == tgt_param:
                continue
            src_cv2 = param_to_cv2[src_param]
            tgt_cv2 = param_to_cv2[tgt_param]
            code_name = f"COLOR_{src_cv2}2{tgt_cv2}"
            code = getattr(cv2, code_name, None)
            if code is not None:
                pairs[(src_param, tgt_param)] = code
