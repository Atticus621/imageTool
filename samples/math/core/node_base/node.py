...
class NodeBase:
    ...
    def _get_input_expressions(self, port_name: str) -> list:
        """从指定端口获取数学表达式列表。"""
        ...
        if data is None:
            return []
        if isinstance(data, MathExpression):
            return [data]
        if isinstance(data, list):
            return [d for d in data if isinstance(d, MathExpression)]
        return []

    def _set_output_expressions(self, port_name: str, expressions: list):
        """向指定端口输出数学表达式列表。"""
        port = self.output_ports.get(port_name)
        if port:
            port.set_data(expressions)

    def _get_input_matrices(self, port_name: str) -> list:
        """从指定端口获取矩阵列表。"""
        ...
        if data is None:
            return []
        if isinstance(data, MathMatrix):
            return [data]
        if isinstance(data, list):
            return [d for d in data if isinstance(d, MathMatrix)]
        return []

    def _set_output_matrices(self, port_name: str, matrices: list):
        """向指定端口输出矩阵列表。"""
        port = self.output_ports.get(port_name)
        if port:
            port.set_data(matrices)
